...
namespace ClickerFixer.Desktop
{
    ...
    public class WindowPositioning
    {
        ...
        public static Point GetWindowPosition(NativeMethods.NOTIFYICONIDENTIFIER nid, double windowwidth, double windowheight, double dpi)
        {
            // retrieve taskbar information
            TaskBarInfo taskbarinfo = GetTaskBarInfo();

            // retrieve notify icon location
            Rect? nipositiontemp = GetNotifyIconRectangle(nid);
        ...
        public static Rect? GetNotifyIconRectangle(NativeMethods.NOTIFYICONIDENTIFIER nid)
        {
            // if (Compatibility.CurrentWindowsVersion == Compatibility.WindowsVersion.Windows7Plus)
                return GetNotifyIconRectWindows7(nid);
            // else
            //     return GetNotifyIconRectLegacy(nid);
        }
        ...
        public static Rect? GetNotifyIconRectWindows7(NativeMethods.NOTIFYICONIDENTIFIER nid)
        {
            if (Compatibility.CurrentWindowsVersion != Compatibility.WindowsVersion.Windows7Plus)
        ...
        public static bool IsNotifyIconInFlyOut(NativeMethods.NOTIFYICONIDENTIFIER nid, TaskBarInfo taskbarinfo)
        {
            if (Compatibility.CurrentWindowsVersion != Compatibility.WindowsVersion.Windows7Plus)
                return false; // nothing to worry about in earlier versions

            Rect? nirect = GetNotifyIconRectangle(nid);
        ...
        public static bool IsPointInNotifyIcon(Point point, NativeMethods.NOTIFYICONIDENTIFIER nid)
        {
            Rect? nirect = GetNotifyIconRectangle(nid);
            if (nirect == null)
                return false;
        ...
        public static bool IsCursorOverNotifyIcon(NativeMethods.NOTIFYICONIDENTIFIER nid)
        {
            return IsPointInNotifyIcon(GetCursorPosition(), nid);
        }
